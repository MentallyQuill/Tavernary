...
1. Open `/help/manage-project/`, select the published personal GitHub
   repository, and choose one exact operation:
   - edit card details
...
3. Create the issue from the GitHub account that currently owns the repository.

### Automation and review flow

1. `admit-issue.yml` dispatches `triage-project-owner-request.yml`.
2. Owner triage verifies the issue author, immutable repository ID, source
   fingerprint, operation, and manifest against current catalog and GitHub
   state.
