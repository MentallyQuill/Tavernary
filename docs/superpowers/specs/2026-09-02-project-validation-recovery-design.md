...
Issues #656 and #658 exhausted three exact-head attempts on the same repository-wide visual regression: the three fork-relationship snapshots differed only in the hard-coded `11d ago` fixture glyph. The card layout, dimensions, responsive behavior, and submission data were unchanged. One #658 attempt also narrowly missed a performance threshold, but the unchanged clean-main performance suite passes well inside the budget, so that one result is runner noise rather than the shared cause.
...
- The visual fixture remains strict while eliminating the single localized glyph-rasterization instability.
...
The failing screenshots have identical diff localization at desktop-standard, desktop-compact, and mobile: only `11d ago` differs. The visual setup already replaces live relative times with fixture text. It will use the already-stable `2d ago` glyph sequence for both cards, retain explicit two-label assertions, keep the existing strict pixel allowance, and refresh only the three affected snapshot bytes. No production CSS or layout changes are involved.
...
1. Merge the workflow, fixture, and Twemoji maintenance change after full local and GitHub verification.
...
- The three fork-relationship visual snapshots pass without raising `maxDiffPixels`.
...
The workflow behavior can be reverted independently by restoring the success predicates and `--exit-status`, although doing so restores the original liveness gap. The visual fixture change affects tests only. Twemoji can be republished by restoring the source to `active`, clearing `status_reason`, and setting `refresh_policy` to `automatic`; its preserved project and Kit records make that reversible.
