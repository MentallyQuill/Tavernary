...
describe("intake migration", () => {
  ...
  test("builds deterministic grammatical summaries", () => {
    ...
    expect(
      provisionalSummary("Pura's Director", "preset", ["sillytavern"]),
    ).toBe("A System Preset for SillyTavern.");
    expect(
      provisionalSummary("Sonder Engine", "frontend", ["sonder-engine"]),
    ).toBe("A frontend for Sonder Engine.");
  });
  ...
  test("fails when an existing provisional record drifts from the deterministic output", () => {
    ...
    ).toThrow("Provisional drift: samueras-guidedgenerations-extension");
  });
